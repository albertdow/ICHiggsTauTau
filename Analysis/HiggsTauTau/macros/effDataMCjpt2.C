TEfficiency* DrawEffs(TString filename)
{
    ...
    // define selection
    TCut offline = "cleanedPFJets_pt.size()=2 & L1Pass & xclean_mjj>1000 & xclean_jpt_1>160 & mva_olddm_medium_1>0.5 & mva_olddm_medium_2>0.5";

    TCut VBF = "trg_VBF";
    TCut DiTau = "trg_doubletau"; 

    // fill the histograms with TTree::Draw
    HLT_trigger_ntuple->Draw("offline_jpt_2>>h_num", VBF && DiTau && offline);
